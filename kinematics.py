...
class SimulatedDeltaBot(object):
    ...
    def getData(bot, step, minServo, maxServo):
        coordinates = []
        angles = []
        for t1 in range(minServo, maxServo, step):
            for t2 in range(minServo, maxServo, step):
                for t3 in range(minServo, maxServo, step):
                    servos = (t1, t2, t3)
                    coordinates.extend([bot.forward(*servos)])
                    there_and_back = bot.reverse(*bot.forward(*servos))
                    angles.extend([there_and_back])
    ...
    def printWorkSpace(coordinates):
        fig = plt.figure()
        ax = fig.add_subplot(1,1,1, projection='3d')
        surf = ax.scatter(xs=[x for x,y,z in coordinates] ,ys=[y for x,y,z in coordinates],zs=[z for x,y,z in coordinates])
        plt.show()
...
if __name__ == "__main__":
    ...
    [coordinates, angles] = SimulatedDeltaBot.getData(bot, step, minServo, maxServo)

    coordinatesCsv = [['X', 'Y', 'Z']]
    coordinatesCsv.extend(coordinates)
    anglesCsv = [['T1', 'T2', 'T3']]
    anglesCsv.extend(angles)
    SimulatedDeltaBot.writeDataToCsv("angles.csv", anglesCsv)
    SimulatedDeltaBot.writeDataToCsv("coordinates.csv", coordinatesCsv)

    SimulatedDeltaBot.printWorkSpace(coordinates)